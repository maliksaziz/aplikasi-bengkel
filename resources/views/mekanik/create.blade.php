@extends('layouts.index')
@section('title', 'Create Mekanik')

@section('content')
<div class="main">
    <div class="main-content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="panel">
                                <div class="panel-heading">
                                    <h3 class="panel-title">Create Data Mekanik</h3>
                                </div>
                                <div class="panel-body">
                                    {{-- menampilkan error validasi --}}
                                    @if (count($errors) > 0)
                                    <div class="alert alert-danger">
                                        <ul>
                                            @foreach ($errors->all() as $error)
                                                <li>{{ $error }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                    @endif
                                <form action="/mekanik/store" method="POST">
                                @csrf
                                <div class="form-body">
                                        <label>ID Alamat </label>
                                        <div class="row">
                                            <div class="col-md-5">
                                                <div class="form-group">
                                                @foreach($dataalm as $almt)
                                                <input type="text" readonly id="id_alamat" class="form-control" name="id_alamat" value="{{$almt->id_alamat + 1}}">
                                                @endforeach
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-5">
                                            <label>Nama Mekanik </label>
                                                <div class="form-group">
                                                <input type="text" id="mekanik" name="mekanik" class="form-control">
                                                </div>
                                            </div>
                                            <div class="col-md-5">
                                            <label>Telepon </label>
                                                <div class="form-group">
                                                <input type="text" id="telp" name="telp" class="form-control">
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="row">
                                            <div class="col-md-10">
                                            <label>Jalan </label>
                                                <div class="form-group">
                                                <input type="text" id="jalan" class="form-control" name="jalan">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4">
                                            <label>Kelurahan </label>
                                                <div class="form-group">
                                                <input type="text" id="kelurahan" class="form-control" name="kelurahan">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                            <label>Kecamatan </label>
                                                <div class="form-group">
                                                <input type="text" id="kecamatan" class="form-control" name="kecamatan">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                            <label>Kota </label>
                                                <div class="form-group">
                                                <input type="text" id="kota" class="form-control" name="kota">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-actions">
                                        <div class="text-right">
                                          <button type="submit" class="btn btn-info pull-right">Create Data</button>
                                        </div>
                                    </div>

                                </form>
                                </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@stop
@section('content1')

                            <div class="panel-heading">
                                    <h3 class="panel-title">Creat Data Mekanik</h3>
                            </div>
                            <div class="panel-body">
                                <form action="/mekanik/store" method="POST">
                                @csrf
                                <div class="form-body">
                                        <label>ID Alamat </label>
                                        <div class="row">
                                            <div class="col-md-5">
                                                <div class="form-group">
                                                @foreach($dataalm as $almt)
                                                <input type="text" readonly id="id_alamat" class="form-control" name="id_alamat" value="{{$almt->id_alamat + 1}}">
                                                @endforeach
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-5">
                                            <label>Nama Mekanik </label>
                                                <div class="form-group">
                                                <input type="text" id="mekanik" name="mekanik" class="form-control">
                                                </div>
                                            </div>
                                            <div class="col-md-5">
                                            <label>Telepon </label>
                                                <div class="form-group">
                                                <input type="text" id="telp" name="telp" class="form-control">
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="row">
                                            <div class="col-md-10">
                                            <label>Jalan </label>
                                                <div class="form-group">
                                                <input type="text" id="jalan" class="form-control" name="jalan">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4">
                                            <label>Kelurahan </label>
                                                <div class="form-group">
                                                <input type="text" id="kelurahan" class="form-control" name="kelurahan">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                            <label>Kecamatan </label>
                                                <div class="form-group">
                                                <input type="text" id="kecamatan" class="form-control" name="kecamatan">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                            <label>Kota </label>
                                                <div class="form-group">
                                                <input type="text" id="kota" class="form-control" name="kota">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-actions">
                                        <div class="text-right">
                                          <button type="submit" class="btn btn-info pull-right">Create Data</button>
                                        </div>
                                    </div>

                                </form>
                            </div>
                    

@stop
@section('content1')
        <!-- ============================================================== -->
        <!-- Page wrapper  -->
        <!-- ============================================================== -->
        <div class="page-wrapper">
            <!-- ============================================================== -->
            <!-- Bread crumb and right sidebar toggle -->
            <!-- ============================================================== -->
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-7 align-self-center">
                        <h4 class="page-title text-truncate text-dark font-weight-medium mb-1">INPUT DATA BENGKEL</h4>
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    <li class="breadcrumb-item"><a href="/dashboard" class="text-muted">Home</a></li>
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Data Mekanik</li>
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Create Data Mekanik</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
            <!-- End Bread crumb and right sidebar toggle -->
            <!-- ============================================================== -->
            <!-- ============================================================== -->
            <!-- Container fluid  -->
            <!-- ============================================================== -->
            <div class="container-fluid">
                <!-- ============================================================== -->
                <!-- Start Page Content -->
                <!-- ============================================================== -->
                <div class="row">
                    <div class="col-10">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Grid With Row Label</h4>
                                <form action="/mekanik/store" method="POST">
                                @csrf
                                    <div class="form-body">
                                        <label>ID Alamat </label>
                                        <div class="row">
                                            <div class="col-md-5">
                                                <div class="form-group">
                                                @foreach($dataalm as $almt)
                                                <input type="text" readonly id="id_alamat" class="form-control" name="id_alamat" value="{{$almt->id_alamat + 1}}">
                                                @endforeach
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-5">
                                            <label>Nama Mekanik </label>
                                                <div class="form-group">
                                                <input type="text" id="mekanik" name="mekanik" class="form-control">
                                                </div>
                                            </div>
                                            <div class="col-md-5">
                                            <label>Telepon </label>
                                                <div class="form-group">
                                                <input type="text" id="telp" name="telp" class="form-control">
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="row">
                                            <div class="col-md-10">
                                            <label>Jalan </label>
                                                <div class="form-group">
                                                <input type="text" id="jalan" class="form-control" name="jalan">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4">
                                            <label>Kelurahan </label>
                                                <div class="form-group">
                                                <input type="text" id="kelurahan" class="form-control" name="kelurahan">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                            <label>Kecamatan </label>
                                                <div class="form-group">
                                                <input type="text" id="kecamatan" class="form-control" name="kecamatan">
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                            <label>Kota </label>
                                                <div class="form-group">
                                                <input type="text" id="kota" class="form-control" name="kota">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-actions">
                                        <div class="text-right">
                                          <button type="submit" class="btn btn-info pull-right">Create Data</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- ============================================================== -->
                <!-- End PAge Content -->
                <!-- ============================================================== -->
            </div>
            <!-- ============================================================== -->
            <!-- End Container fluid  -->
            <!-- ============================================================== -->
        </div>
        <!-- ============================================================== -->
        <!-- End Page wrapper  -->
        <!-- ============================================================== -->
@endsection